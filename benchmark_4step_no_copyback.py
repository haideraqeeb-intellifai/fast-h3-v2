"""Experimental four-forward FastH3 V2 benchmark: one warm-up and one measured request."""
from pathlib import Path
from publication_runtime import require_variant, fastvideo_revision
import dataclasses
import gc
import hashlib
import json
...
OUT = ROOT / 'run_4step_no_copyback'
...
def main():
    ...
    args = basic_fasth3.parse_args([
        '--model-path', str(ROOT / 'models/FastH3-V2-experimental-4-step'),
        '--prompt', prompt, '--output', str(OUT),
        '--height', '1280', '--width', '736', '--num-frames', '243',
        '--seed', '42', '--warmup-seed', '42', '--repeats', '1',
        '--steps', '5', '--vsa-sparsity', '0.8', '--vsa-tile-size', '64',
    ...
    report = {
        ...
        'task_started_utc': '2026-09-16T15:33:45Z',
        'model_id': 'FastVideo/FastVideo-FastH3-8-Step-V2',
        'model_revision': '3da2ddfe1954d9cda4c05b643dc0f26007a655c5',
        'fastvideo_commit': fastvideo_revision(),
        'runtime_patch_sha256': hashlib.sha256((ROOT / 'work/cpu-load.patch').read_bytes()).hexdigest(),
        'decoder_policy': 'retain_cpu_weights_discard_gpu_copies',
        'decoder_patch_sha256': hashlib.sha256((ROOT / 'work/decoder-no-copyback.patch').read_bytes()).hexdigest(),
        ...
        'experiment': json.loads((ROOT / 'models/FastH3-V2-experimental-4-step/fastvideo_inference.json').read_text()),
        'runs': [],
    }
